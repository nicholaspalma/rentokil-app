...
# --- GESTIÓN DE ESTADO PROFUNDA (ANTI-BORRADO) ---
...
if "pdf_dialogo" not in st.session_state:
    st.session_state.pdf_dialogo = None
...
def procesar_imagen_full(uploaded_file):
    """Procesa imagen para el informe de diálogo manteniendo aspecto original"""
    try:
        uploaded_file.seek(0)
        image = Image.open(uploaded_file)
        image = ImageOps.exif_transpose(image)
        ...
        # Reducir si es absurdamente gigante, pero manteniendo proporción
...
if st.session_state.app_mode == "HOME":
    ...
    with c3:
        if st.button("📸 INFORME DE DIÁLOGO\n(Fotos a Pantalla Completa)", use_container_width=True):
            st.session_state.app_mode = "DIALOGO"
            st.rerun()
    with c4:
        if st.button("📄 CONVERTIR PDF A WORD\n(Formato DOCX)", use_container_width=True):
            st.session_state.app_mode = "PDF2WORD"
            st.rerun()
...
# ==============================================================================
    ...
    with cc3:
        inf_ref_mol = st.text_input("Informe Ref.", f"2026-{num_cert} NP")
    ...
    # ESCRITURA EN CACHE DIRECTA PARA EVITAR BORRADO
    ...
    if st.button("🚀 GENERAR INFORME Y CERTIFICADO", use_container_width=True, type="primary"):
        ...
        try:
            ...
            if fotos_sellado_mol:
                pdf.galeria(fotos_sellado_mol, "Evidencia de Sellado:")
            ...
            pdf.ln(2)
            # Anchos recalibrados para sumar 190
            ...
            for col in df_m_val.columns[2:]:
                ax.plot(e_x, pd.to_numeric(df_m_val[col], errors='coerce'), marker='o', label=col)
            ...
            pdf.ln(5)
            # Anchos recalibrados para sumar 190
            pdf.tabla(list(df_m_val.columns), [[str(x) for x in r] for _, r in df_m_val.iterrows()], [25, 15, 25, 25, 25, 25, 25, 25])
...
# ==============================================================================
    ...
    with cc2:
        ingrediente = st.selectbox("Fumigante a Declarar", ["Fosfuro de Aluminio (AIP) 56%", "Fosfuro de Magnesio", "Mixto"])
    with cc3:
        inf_ref_est = st.text_input("Informe Ref.", f"2026-{num_cert} NP")
    ...
    if st.button("🚀 GENERAR INFORME Y CERTIFICADO", use_container_width=True, type="primary"):
        ...
        try:
            ...
            # Anchos recalibrados para sumar 190
            pdf.tabla([str(c) for c in df_med_est_val.columns], [[str(x) for x in r] for _, r in df_med_est_val.iterrows()], [25, 15, 30, 30, 30, 30, 30])
...
# ==============================================================================
    ...
    st.markdown("Este módulo genera un PDF oficial con el encabezado Rentokil, donde cada fotografía subida ocupa el **máximo espacio posible en una página entera**.")
    
    col_d1, col_d2 = st.columns(2)
    with col_d1:
        cli_d = st.text_input("Cliente / Razón Social", "Empresa Ejemplo S.A.")
        pla_d = st.text_input("Planta o Instalación", "Planta Central")
    with col_d2:
        dir_d = st.text_input("Dirección", "Av. Siempre Viva 123")
        fec_d = st.date_input("Fecha", datetime.date.today())
        
    fotos_dialogo = st.file_uploader("Sube TODAS las fotos aquí", accept_multiple_files=True, type=['png','jpg','jpeg','heic'])
    ...
    if st.button("🚀 GENERAR INFORME DE DIÁLOGO", use_container_width=True, type="primary"):
        try:
            ...
            pdf.set_font("Arial", "", 11)
            pdf.cell(35, 7, "Cliente:", 0)
            pdf.cell(0, 7, str(cli_d), 0, ln=1)
            pdf.cell(35, 7, "Planta:", 0)
            pdf.cell(0, 7, str(pla_d), 0, ln=1)
            pdf.cell(35, 7, "Dirección:", 0)
            pdf.cell(0, 7, str(dir_d), 0, ln=1)
            pdf.cell(35, 7, "Fecha:", 0)
            pdf.cell(0, 7, format_fecha_es(fec_d), 0, ln=1)
            ...
            if fotos_dialogo:
                for f in fotos_dialogo:
                    ...
                    if tmp_p:
                        ...
                        max_ratio = 190 / 240
                        if ratio > max_ratio:
                            pdf.image(tmp_p, x=10, y=35, w=190)
                        else:
                            pdf_w = 240 * ratio
                            pdf_x = 10 + (190 - pdf_w) / 2
                            pdf.image(tmp_p, x=pdf_x, y=35, h=240)
                        os.remove(tmp_p)
            
            with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp_d:
                pdf.output(tmp_d.name)
                with open(tmp_d.name, "rb") as fd:
                    st.session_state.pdf_dialogo = fd.read()
            st.rerun()
        except Exception as e:
            st.error(f"Error generando Diálogo: {e}")
...
# ==============================================================================
    ...
    if not PDF2DOCX_INSTALLED:
        st.error("⚠️ Falta una librería. Por favor, asegúrate de tener `pdf2docx` en tu archivo `requirements.txt` de GitHub.")
    else:
        st.markdown("Sube cualquier archivo PDF para convertirlo automáticamente a formato editable de Word (`.docx`).")
        ...
        if uploaded_pdf and st.button("🔄 CONVERTIR A WORD", use_container_width=True, type="primary"):
            with st.spinner("Convirtiendo documento... Esto puede tardar unos segundos..."):
                try:
                    ...
                    st.error(f"Hubo un error durante la conversión: {e}")

# ==============================================================================
# BOTONES DE DESCARGA GLOBALES (MOLINOS Y ESTRUCTURAS)
# ==============================================================================
