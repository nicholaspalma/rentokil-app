...
# ==============================================================================
    ...
    if st.button("🚀 GENERAR INFORME MOLINOS"):
        try:
            ...
            pdf.cell(0, 6, str(cliente), 0, ln=1)
            pdf.cell(30, 6, "Planta:", 0)
            pdf.cell(0, 6, f"{planta} - {direccion}", 0, ln=1)
            pdf.cell(30, 6, "Atención:", 0)
            pdf.cell(0, 6, str(atencion), 0, ln=1)
            pdf.cell(30, 6, "Fecha:", 0)
            pdf.cell(0, 6, str(fecha_inf), 0, ln=1)
            ...
            if ruta_firma:
                try:
                    x_c = (210 - 60) / 2
                    pdf.image(ruta_firma, x=x_c, w=60)
...
# ==============================================================================
    ...
    if st.button("🚀 GENERAR INFORME ESTRUCTURAS"):
        try:
            ...
            pdf.set_font("Arial", "", 10)
            pdf.cell(30, 6, "Cliente:", 0)
            pdf.cell(0, 6, str(cliente_e), 0, ln=1)
            pdf.cell(30, 6, "Dirección:", 0)
            pdf.cell(0, 6, str(direccion_e), 0, ln=1)
            pdf.cell(30, 6, "Tratamiento:", 0)
            pdf.cell(0, 6, f"{tipo_trat} - Plaga: {plaga_e}", 0, ln=1)
            fecha_str_larga = fecha_e.strftime("%d-%m-%Y")
            pdf.cell(30, 6, "Fecha:", 0)
            pdf.cell(0, 6, fecha_str_larga, 0, ln=1)
            ...
            for index, row in df_estructuras.iterrows():
                try:
                    ...
                    if vol > 0 or n_pla > 0 or n_rop > 0 or n_pho > 0:
                        g_row = (n_pla * 33) + (n_rop * 333) + (n_pho * 1)
                        dosis_row = g_row / vol if vol > 0 else 0
                        total_g += g_row; total_vol += vol
            ...
            pdf.multi_cell(0, 6, concl_text_est)
            pdf.ln(20)
